defmodule FaceCheckin.ProfileFacesTest do
  ...
  test "get_profile_face!/1 returns the profile_face with given id", %{profile: profile, face: face} do
    {:ok, pf} = ProfileFaces.create_profile_face(Map.merge(@valid_attrs, %{profile_id: profile.id, face_id: face.id}))
    assert ProfileFaces.get_profile_face!(pf.id) == pf
  end

  test "create_profile_face/1 with valid data creates a profile_face", %{profile: profile, face: face} do
  ...
  test "update_profile_face/2 with invalid data returns error changeset", %{profile: profile, face: face} do
    {:ok, pf} = ProfileFaces.create_profile_face(Map.merge(@valid_attrs, %{profile_id: profile.id, face_id: face.id}))
